 ...
 * 1. Wide asphalt street with pedestrian zebra crossing & driving cars with exhaust puffs.
 * 2. Stage 1 Sidewalk Boutique Kiosk with yellow awning & true 3D Conical Patio Umbrellas.
 * 3. Stage 2 Fashion Van (Custom mobile boutique food truck).
 * 4. Typography: FONT_FAMILY ('Fredoka', 'Nunito', sans-serif).
 */
...
export class TrafficManager {
  constructor(scene, parentContainer) {
    ...
    this.carModels = [
      { body: 0xef4444, roof: 0xb91c1c },
      { body: 0xf59e0b, roof: 0xd97706 },
      { body: 0x0ea5e9, roof: 0x0284c7 },
      { body: 0x8b5cf6, roof: 0x6d28d9 },
      { body: 0x10b981, roof: 0x047857 }
    ];
    ...
    scene.time.delayedCall(600, () => this.spawnCar());
  }

  spawnCar() {
    const model = Phaser.Utils.Array.GetRandom(this.carModels);
    const laneY = Phaser.Math.Between(0, 1) === 0 ? 68 : 112;
...
/**
 * Environment Manager: Renders Stage 1 (Sidewalk Kiosk) and Stage 2 (Fashion Van)
 */
export class EnvironmentManager {
  ...
  drawStage1Boutique() {
    ...
    // Inlay runner rug framing counter and sewing station
    ...
    // True 3D Conical Patio Umbrellas Flanking the Counter
    this.umbrellas.push(drawTrue3DUmbrella(this.scene, this.container, GAME_CONFIG.layout.umbrellas.left.x, GAME_CONFIG.layout.umbrellas.left.y));
    this.umbrellas.push(drawTrue3DUmbrella(this.scene, this.container, GAME_CONFIG.layout.umbrellas.right.x, GAME_CONFIG.layout.umbrellas.right.y));
  ...
  drawStage2FashionVan() {
    ...
    // Ground Drop Shadow under entire Van
...
/**
 * True 3D Conical Patio Umbrella with Curved Blue/White Striped Wedges
 * and soft ambient drop shadows.
 */
export function drawTrue3DUmbrella(scene, parentContainer, x, y) {
  ...
  // 1. Soft Ambient Drop Shadow on Ground
  g.fillStyle(0x000000, 0.28);
  g.fillEllipse(0, 64, 60, 24);

  // 2. Heavy Cast Iron Base
  g.fillStyle(0x334155, 1.0);
  g.fillCircle(0, 58, 16);
  g.fillStyle(0x1e293b, 1.0);
  g.fillCircle(0, 58, 9);

  // 3. Teak Wood / Steel Pole
  g.fillStyle(0x64748b, 1.0);
  g.fillRect(-3, -20, 6, 78);
  g.fillStyle(0x94a3b8, 1.0);
  g.fillRect(-1, -20, 2, 78);

  // 4. True 3D Conical Parasol Canopy
  // Conical Apex at (0, -26), Base ellipse centered at (0, 8) with rx: 55, ry: 32
  const apexY = -26;
  const baseY = 8;
  const rx = 55;
  const ry = 32;
  const segments = 10;
  const colors = [0x0284c7, 0xffffff]; // Vibrant Sky Blue & Crisp White
  ...
  for (let i = 0; i < segments; i++) {
    ...
    const y1 = baseY + Math.sin(a1) * ry;
    const x2 = Math.cos(a2) * rx;
    const y2 = baseY + Math.sin(a2) * ry;
    ...
    // 3D Depth shading on lower half
    const midY = (y1 + y2) / 2;
    if (midY > baseY) {
    ...
    // Curved scalloped bottom valance
    g.fillStyle(color, 1.0);
    g.fillCircle((x1 + x2) / 2, (y1 + y2) / 2 + 2, 5);
  }

  // Brass Cap / Finial on top apex
  g.fillStyle(0xf59e0b, 1.0);
  g.fillCircle(0, apexY - 3, 5);
  g.fillStyle(0xfef08a, 1.0);
  g.fillCircle(-1, apexY - 4, 2);
